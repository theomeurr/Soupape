...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  // Persist on every change (debounced to the next frame to batch bursts).
  const frame = useRef<number | null>(null);
  useEffect(() => {
    if (frame.current) cancelAnimationFrame(frame.current);
    frame.current = requestAnimationFrame(() => saveData(data));
    return () => {
      if (frame.current) cancelAnimationFrame(frame.current);
    };
  }, [data]);
  ...
  const add = useCallback(<K extends CollectionKey>(key: K, entry: Omit<EntryMap[K], 'id'>) => {
    ...
    setData((prev) => ({
      ...prev,
      [key]: [...(prev[key] as EntryMap[K][]), { ...entry, id } as EntryMap[K]],
    }));
    return id;
  }, []);
  ...
  const update = useCallback(
    <K extends CollectionKey>(key: K, id: string, patch: Partial<EntryMap[K]>) => {
      setData((prev) => ({
        ...prev,
        [key]: (prev[key] as EntryMap[K][]).map((e) => (e.id === id ? { ...e, ...patch } : e)),
      }));
    },
    [],
  );
